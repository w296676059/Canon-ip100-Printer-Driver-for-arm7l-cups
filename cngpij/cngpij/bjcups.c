...
static short getDeviceURI(const char *pDestName, char *pDeviceURI, short bufSize)
{
    char                    *pDUri = NULL;    // Device URI.
    http_t                  *pHTTP = NULL;    // HTTP connection.
    ipp_t                   *pRequest = NULL; // IPP Request.
    ipp_t                   *pResponse = NULL; // IPP Response.
    ipp_attribute_t         *pAttribute = NULL; //IPP Attribute.
    char                    *pPrinter = NULL; // Printer Name.
    ...
    // Build a GET_PRINTERS request.
    pRequest = ippNewRequest(IPP_OP_GET_PRINTERS);
    ippSetOperation(pRequest, CUPS_GET_PRINTERS); // Use API function
    ippSetRequestId(pRequest, 1); // Use API function

    // Do IPP request.
    pResponse = cupsDoRequest(pHTTP, pRequest, "/");

    if (ippGetStatusCode(pResponse) > IPP_OK_CONFLICT) { // Use API function
        httpClose(pHTTP);
        ippDelete(pResponse);
        return -1;
    }

    pAttribute = ippFirstAttribute(pResponse); // Use API function
    if (pAttribute != NULL) {
        while (pAttribute != NULL && ippGetGroupTag(pAttribute) != IPP_TAG_PRINTER) { // Use API function
            pAttribute = ippNextAttribute(pResponse); // Use API function
        }
        while (pAttribute != NULL && ippGetGroupTag(pAttribute) == IPP_TAG_PRINTER) { // Use API function
            if (!strcmp(ippGetName(pAttribute), "printer-name") && ippGetValueTag(pAttribute) == IPP_TAG_NAME) { // Use API functions
                pPrinter = (char*)ippGetString(pAttribute, 0, NULL); // Cast to remove const
            }
            if (!strcmp(ippGetName(pAttribute), "device-uri") && ippGetValueTag(pAttribute) == IPP_TAG_URI) { // Use API functions
                pDUri = (char*)ippGetString(pAttribute, 0, NULL); // Cast to remove const
            }
            pAttribute = ippNextAttribute(pResponse); // Use API function
        }
    }
    ...
    if (pDUri != NULL) {
        if (strncmp(pDUri, "usb", 3) == 0) {
            if (strcmp(pPrinter, pDestName) == 0) {
                httpClose(pHTTP);
                ippDelete(pResponse);
                snprintf(pDeviceURI, bufSize, "%s", pDUri);
                return 0;
            }
        }
    }
