	...
	char 				*modelname )
{
	...
	if (rc < -1) {
		...
		if(rc == POPT_ERROR_BADOPT){
			fprintf(stderr, "Try \"--help\" option for more information.\n");
		}
		goto onError;
	}
	...
	*/
	...
	if (setopt[OPTINDEX(OPTTONE)] & OPTBIT(OPTTONE)) {
		if( formattype < CND_FORMATTYPE_11 || !tone_support){	/* not support tone */
			...
			fprintf(stderr, "Error: inappropriate media selection.\n");
			/* inappropriate selection error. */
			goto onError;
			
		} else if ( !is_grayscale_mode ){	/* not grayscale mode */
			fprintf(stderr, "Error: Cannot use \"--tone\" without \"--grayscale\" \n");
			/* inappropriate selection error. */
			goto onError;
