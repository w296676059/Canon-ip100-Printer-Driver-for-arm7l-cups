...
static void SetBJFltDevice(LPBJFLTDEVICE bjdev)
{
	...
	/* Ver.3.60 : Disable bjfltBinMethod */
	type = CNCL_GetTableFormatType(&g_uidb.nominfo, (void *)g_bjlibdir);	/* Ver.2.70 */
	if ( type >= CND_FORMATTYPE_10 ){
		bjdev->bjfltBinMethod = CND_UIBIN_NA;
	}
...
static void SetBJFltColorSystem2(LPBJFLTCOLORSYSTEM bjcolor)
{
	...
	if( IsGrayPrint(g_main_window) )	/* Grayscale = ON */
	{
		...
		/* Ver.3.40 */
		bjcolor->bjfltMonochromeTone = g_color_dialog2->tone_balance;
	}
	else								/* Grayscale = OFF */
	{
...
//	gchar* alert_msg = LookupText(g_keytext_list, "move_lever_position_alert");
	...
	position_alert_key_len = strlen(position_key) + strlen(_alert_str) + 1;
	gchar* position_alert_key = g_malloc0(position_alert_key_len);

	strncpy(position_alert_key, position_key, position_alert_key_len);
	position_alert_key[position_alert_key_len -1] = '\0';
	...
	msg_buf_len = strlen(alert_msg) + strlen(position_alert_msg) + 1; /* Ver.2.80 "1":\0 */
	msg_buf = (gchar*)g_malloc( msg_buf_len );

	snprintf(msg_buf, msg_buf_len, alert_msg, position_alert_msg); /* Ver.3.60 */

	DisableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
	UtilMessageBox(msg_buf, g_window_title, MB_OK | MB_ICON_INFORMATION, NULL);
                                        ...
                                        gpointer         user_data)
{
	...
	if( duplex_value == CND_DUPLEX_AUTO ){
		if( !IsAvailableSizeDuplex() )		/* mismatch!! */
		{
			...
			change_obj = LookupText(g_keytext_list, "LUM_IDD_PAG_PAGESET_XP_MANUDUP_AUTODUP_IDC_CHK_DUPLEX");
			...
			DisableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
			UtilMessageBox( message,g_window_title, MB_ICON_NO | MB_OK, NULL);
			EnableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
			g_free( message );
			g_free( tmp_message );
			return;
		...
		for ( i=0; i< g_uidb.dbsize; i++, lpdb++ ){
			if ( (lpdb->nObjectID == CNCL_LOWER_PAPERSUPPORT_SIZE) 
				&& (lpdb->nValue == bjdev.bjfltPaperSize)){
					...
					DisableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
					UtilMessageBox( dialog_msg1,g_window_title, MB_ICON_NO | MB_OK, NULL);
					EnableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
			}
		}
	}	
	...
	/* Ver.3.60:Media setting guide */
	if ( IsAvailableValue( CNCL_MEDIASUPPLY, CND_SUPPLY_CASSETTE_06 ) ) {
		gchar *msg1, *msg2, *msg;
		gint msgLen;

		msg1 = LookupText(g_keytext_list, "LUM_MessTitlePaperLoadGuide");
		msg2 = LookupText(g_keytext_list, "LUM_MessContentsPaperLoadGuide");
		msgLen = strlen(msg1) + strlen(msg2) + 2;
		msg = (gchar*)g_malloc( msgLen );
		snprintf( msg, msgLen, "%s\n%s", msg1, msg2 );

		if ( GetCurrentnValue( CNCL_MEDIATYPE) !=  CND_MEDIA_PLAIN ) {
			DisableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
			if ( UtilMessageBox( msg, g_window_title, MB_ICON_NO | MB_OK | MB_CANCEL, NULL) != US_OK ) {
				g_free( msg );
				EnableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
				return;	
			}
...
on_util_clean_button_clicked           (GtkButton       *button,
                                        gpointer         user_data)
{
//	init_nozzle_check_type_a_value(); 2010
//	UtilCleaning(&g_uidb);
...
update_by_media_type_entry()
{
	...
	if( old_supply != new_supply )
	{
		...
		DisableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
		UtilMessageBox(message,g_window_title, MB_ICON_EXCLAMATION | MB_OK, NULL);
		EnableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
		...
		if( GTK_TOGGLE_BUTTON(button)->active )
		{
			...
			if( IsAvailableMediaBorder(g_mediaborder_dialog, media_type_value) )
			{
				...
				UtilMessageBox(alert_msg, g_window_title,
					MB_OK | MB_ICON_INFORMATION, NULL);
				EnableSensitiveDialog( (GtkWidget *)(UI_DIALOG(g_main_window)->window) );
