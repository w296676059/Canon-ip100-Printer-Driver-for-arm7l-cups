...
int LoadResources()
{
	...
	while( 1 )
	{
		...
		{
			gint glade_path_len = strlen(PACKAGE_DATA_DIR) + 1 + strlen(glade_name) + 1;
			char* glade_path = g_malloc0( glade_path_len );

			strncpy(glade_path, PACKAGE_DATA_DIR, glade_path_len);
			glade_path[glade_path_len -1] = '\0';
